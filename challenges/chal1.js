...
/*
Challenge 2:
Write a function that uses the Array.prototype.map() method to console.log the first and last names of each president from the given 'presidents' array.
*/
...
const fnMap = () => {
    return presidents.map (({first, last}) => `${first
     } ${last}`);
    };
     console.log(fnMap());
...
const recPres = presidents.filter((birth) => birth.year > 1899 );
console.log(recPres);

// challenge 4:

/*
Write a function that uses the the Array.prototype.reduce() method and find the total number of years that presidents have lived. You will need to implement logic that checks if the president is still currently living. If the president IS still currently living, use the current year of 2021. The result returned should total 3170.
*/

const redPres = presidents.reduce()
